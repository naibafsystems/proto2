...
class ReportesTotalizados extends CI_Controller { 
    ...
    public function index() {
        ...
        $datos["contenido"] = "administrador/reportesTotalizados";
	...
	public function cargaReporte() {		
		...
		$convocatoria = trim($this->input->post('convocatoria'));
		...
		if($operativo != '' || $encuesta != '' || $rol != '' || $ciudad != ''){				
			
			$datos["reporte"] = $this->convocatorias_model->reporteTotalizadoAbierta($operativo, $encuesta, $rol, $ciudad, $convocatoria);
			
			if(count($datos["reporte"]) > 0){
        		$this->load->view('administrador/reportesTotalizadosDatos', $datos);
				//echo $data;
			}else{
