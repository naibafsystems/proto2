...
class ReporteConvCerradas extends CI_Controller { 
    ...
    public function index() {
        ...
        $datos["contenido"] = "coordinador/reporteConvCerradas";
	...
	public function cargaReporte() {		 
		...
		if($operativo != '' || $encuesta != '' || $rol != '' || $cedula != '' || $nombreC != ''){				
			
			$datos["reporte"] = $this->convocatorias_model->reporteConvCerradas($operativo, $encuesta, $rol, $ciudades, $cedula, $nombreC);
			
			if(count($datos["reporte"]) > 0){
        		$this->load->view('coordinador/reporteConvCerradasDatos', $datos);
			}else{
